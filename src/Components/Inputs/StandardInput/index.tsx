...
const StandardInput = (props: StandardInputProps) => {
    ...
    return (
        <View style={{
            ...
            />
            {
                onPasswordEyePress && <View style={{position: "absolute", right: 10, bottom: 0, justifyContent: "center", height: "100%"}}>
                    <Pressable onPress={onPasswordEyePress}>
                        {showPassword ? <MaterialCommunityIcons name="eye-outline" color={colors.white} size={20} /> : <MaterialCommunityIcons name="eye-off-outline" color={colors.white} size={20} />}
                    </Pressable>
                </View>
            }
        </View>
