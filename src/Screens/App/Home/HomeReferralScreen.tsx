...
const HomeReferralScreen: FC<HomeReferralScreenNavigationProp> = ({
	...
	const onRuleOfReferralCloseBtnPress = () =>
			...
			>
				...
				>
					...
					>
						<Text
							style={[
								{ color: colors.white, paddingTop: 0, paddingHorizontal: 20 },
								Fonts.textSmall,
							]}
						>
							{t('yourReferralCode')}
				...
				>
					<View style={{ flex: 4, flexDirection: 'row', alignItems: 'center' }}>
						<Text
							style={[
								...
								{ color: colors.white, fontSize: 18, marginRight: 6 },
							]}
						>
							{t('totalReferred')}
						</Text>
					...
					<Text style={[ Fonts.textSmall, { color: colors.brightTurquoise } ]}>
						{referralInfo.referred}{' '}
						{t(`friend${referralInfo.referred <= 1 ? '' : 's'}`)}
					</Text>
				</View>
				...
				<View style={{ height: 80, alignItems: 'flex-start', width: '85%' }}>
					{referredNames.map((elem, idx) => {
						return (
							<View
								style={[ REFERRED_FRIEND_ICON, { top: 0, left: idx * 35 } ]}
								key={`Friend-${idx}`}
							>
								<Text
									style={{
				...
				>
					...
					/>
					<Text style={[ { fontSize: 14, color: colors.white, flexShrink: 1 } ]}>
						{t('earnKEWhenReferredFriends')}
					</Text>
				</View>
